  ...
  return (function e(t, n, r) {
  ...
  })(
    {
      ...
      9: [
        ...
        { "../utils": 14, "./getusermedia": 10, "rtcpeerconnection-shim": 2 },
      ],
      10: [
        function (require, module, exports) {
          module.exports = function (window) {
...
var enums = {
  EventType: { ICE_CONNECTION_STATE_CONNECTED: 3, ROOM_SIZE_2: 2 },
  RequestField: {
  ...
  ClientType: { UNKNOWN: 0, ANDROID: 4, DESKTOP: 2, IOS: 3, JS: 1 },
};
...
var AppController = function (loadingParams) {
  ...
  this.newRoomButton_.addEventListener(
    "click",
    this.onNewRoomClick_.bind(this),
    false
  );
  this.rejoinButton_.addEventListener(
    "click",
    this.onRejoinClick_.bind(this),
    false
  );
  this.muteAudioIconSet_ = new AppController.IconSet_(
    UI_CONSTANTS.muteAudioSvg
  );
  this.muteVideoIconSet_ = new AppController.IconSet_(
    UI_CONSTANTS.muteVideoSvg
  );
  this.fullscreenIconSet_ = new AppController.IconSet_(
    UI_CONSTANTS.fullscreenSvg
  );
    ...
    .then(
      function (newParams) {
        ...
        if (this.loadingParams_.roomId) {
          this.createCall_();
          if (
            !RoomSelection.matchRandomRoomPattern(this.loadingParams_.roomId)
          ) {
...
AppController.prototype.createCall_ = function () {
  ...
  this.call_.onsignalingstatechange = this.infoBox_.updateInfoDiv.bind(
    this.infoBox_
  );
  this.call_.oniceconnectionstatechange = this.infoBox_.updateInfoDiv.bind(
    this.infoBox_
  );
  this.call_.onnewicecandidate = this.infoBox_.recordIceCandidateTypes.bind(
    this.infoBox_
  );
...
AppController.prototype.waitForRemoteVideo_ = function () {
  if (this.remoteVideo_.readyState >= 2) {
    trace(
      "Remote video started; currentTime: " + this.remoteVideo_.currentTime
    );
    this.transitionToActive_();
  } else {
...
AppController.prototype.transitionToActive_ = function () {
  ...
  trace(
    "Call setup time: " +
      (connectTime - this.call_.startTime).toFixed(0) +
      "ms."
  );
...
AppController.prototype.transitionToWaiting_ = function () {
  ...
  if (!this.remoteVideoResetTimer_) {
    this.remoteVideoResetTimer_ = setTimeout(
      function () {
        this.remoteVideoResetTimer_ = null;
...
AppController.prototype.pushCallNavigation_ = function (roomId, roomLink) {
  if (!isChromeApp()) {
    window.history.pushState(
      { roomId: roomId, roomLink: roomLink },
      roomId,
      roomLink
    );
  }
...
Call.prototype.hangup = function (async) {
  ...
  if (async) {
    ...
    for (var i = 0; i < steps.length; ++i) {
      promise = promise
        .then(steps[i].step)
        .catch(errorHandler.bind(this, steps[i].errorString));
    }
    return promise;
  }
...
Call.prototype.connectToRoom_ = function (roomId) {
  ...
  var channelPromise = this.channel_.open().catch(
    function (error) {
      this.onError_("WebSocket open error: " + error.message);
      return Promise.reject(error);
    }.bind(this)
  );
  var joinPromise = this.joinRoom_()
    .then(
      function (roomParams) {
  ...
  Promise.all([channelPromise, joinPromise])
    .then(
      function () {
  ...
  if (needStream) {
    ...
    mediaPromise = navigator.mediaDevices
      ...
      .then(
        function (stream) {
          trace(
            "Got access to local media with mediaConstraints:\n" +
              "  '" +
    ...
    iceServerPromise = requestIceServers(
      ...
      .then(
        function (iceServers) {
          ...
          this.params_.peerConnectionConfig.iceServers = servers.concat(
            iceServers
          );
        }.bind(this)
      )
...
Call.prototype.onRecvSignalingChannelMessage_ = function (msg) {
  this.maybeCreatePcClientAsync_().then(
...
var InfoBox = function (infoDiv, call, versionInfo) {
  ...
  this.remoteVideo_.addEventListener(
    "playing",
    function (event) {
...
InfoBox.prototype.refreshStats_ = function () {
  this.call_.getPeerConnectionStats(
    function (response) {
    ...
    if (localAddr && remoteAddr) {
      ...
      contents += this.buildLine_(
        ...
        localAddr +
          " (" +
          localAddrType +
          (typeof localTypePref !== undefined
...
InfoBox.prototype.buildStatsSection_ = function () {
  ...
  var prevStatReport = enumerateStats(
    this.prevStats_,
    this.localTrackIds_,
    this.remoteTrackIds_
  );
  ...
  if (
    ...
    txVideoFps = calculateFps(
      this.remoteVideo_,
      this.remoteDecodedFrames_,
      this.remoteStartTime_,
      "local",
      this.updateDecodedFramesCallback_
    );
...
var PeerConnectionClient = function (params, startTime) {
  ...
  trace(
    "Creating RTCPeerConnnection with:\n" +
      "  config: '" +
      JSON.stringify(params.peerConnectionConfig) +
      "';\n" +
      "  constraints: '" +
      JSON.stringify(params.peerConnectionConstraints) +
      "'."
  );
  ...
  this.pc_.oniceconnectionstatechange = this.onIceConnectionStateChanged_.bind(
    this
  );
...
PeerConnectionClient.prototype.receiveSignalingMessage = function (message) {
  ...
  if (
    (this.isInitiator_ && messageObj.type === "answer") ||
...
PeerConnectionClient.prototype.close = function () {
  ...
  this.pc_.close();
  window.dispatchEvent(
    new CustomEvent("pcclosed", { detail: { pc: this, time: new Date() } })
  );
  this.pc_ = null;
};
...
PeerConnectionClient.prototype.processSignalingMessage_ = function (message) {
  if (message.type === "offer" && !this.isInitiator_) {
    if (this.pc_.signalingState !== "stable") {
      trace(
        "ERROR: remote offer received in unexpected state: " +
    ...
    if (message.type === "answer" && this.isInitiator_) {
      if (this.pc_.signalingState !== "have-local-offer") {
        trace(
          "ERROR: remote answer received in unexpected state: " +
            this.pc_.signalingState
        );
        return;
      }
...
PeerConnectionClient.prototype.onIceConnectionStateChanged_ = function () {
  ...
  if (this.pc_.iceConnectionState === "completed") {
    trace(
      "ICE complete time: " +
        (window.performance.now() - this.startTime_).toFixed(0) +
        "ms."
    );
  }
...
PeerConnectionClient.prototype.recordIceCandidate_ = function (
  location,
  candidateObj
) {
...
RemoteWebSocket.prototype.handleMessage_ = function (message) {
  if (
    message.action === Constants.WS_ACTION &&
...
RoomSelection.prototype.buildRecentRoomList_ = function (recentRooms) {
  ...
  for (var i = 0; i < recentRooms.length; ++i) {
    ...
    href.addEventListener(
      "click",
      this.makeRecentlyUsedClickHandler_(recentRooms[i]).bind(this),
      false
    );
  }
};
      ...
      this.getRecentRooms()
        .then(
          function (recentRooms) {
            ...
            this.storage_.setStorage(
              this.RECENTROOMSKEY_,
    ...
    function (resolve, reject) {
      ...
      this.websocket_.onopen = function () {
        ...
        this.websocket_.onclose = function (event) {
          trace(
            "Channel closed with code:" + event.code + " reason:" + event.reason
          );
          this.websocket_ = null;
          this.registered_ = false;
        };
...
SignalingChannel.prototype.close = function (async) {
  ...
  return sendUrlRequest("DELETE", path, async)
    .catch(
      function (error) {
        ...
        switch (report.type) {
          ...
          case "inbound-rtp":
            if (report.hasOwnProperty("trackId")) {
              if (report.trackId.indexOf(remoteTrackIds.audio) !== -1) {
                ...
                statsObject.audio.remote.packetsReceived =
              ...
              if (report.trackId.indexOf(remoteTrackIds.video) !== -1) {
                ...
                statsObject.video.remote.packetsReceived =
                  report.packetsReceived;
          ...
          case "candidate-pair":
            if (report.hasOwnProperty("availableOutgoingBitrate")) {
              ...
              statsObject.connection.remoteCandidateId =
              ...
              statsObject.connection.responsesReceived =
      ...
      function (report) {
        switch (report.type) {
          ...
          case "local-candidate":
            if (report.hasOwnProperty("id")) {
              if (
                report.id.indexOf(statsObject.connection.localCandidateId) !==
                -1
              ) {
          ...
          case "remote-candidate":
            if (report.hasOwnProperty("id")) {
              if (
                report.id.indexOf(statsObject.connection.remoteCandidateId) !==
                -1
              ) {
...
function computeRate(newReport, oldReport, statName) {
  ...
  return (
    ((newVal - oldVal) / (newReport.timestamp - oldReport.timestamp)) * 1000
  );
}
